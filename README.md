...
Не добавляйте пользовательские сохранения в Git. На крупном размере текста
доступна вертикальная прокрутка; фокус клавиатуры раскрывает нужный элемент. Для разработки и проверки:
...
Точные версии инструментов и их зависимостей записаны в `requirements-dev.lock`.
Для воспроизводимого окружения установите этот файл перед `pip install -e .`.
...
- Тёмное оформление с векторными эмблемами классов, выделением активного бойца и отдельной хроникой боя.
...
- Посмотреть журнал, результат и историю последних 50 матчей с фильтрами и страницами.
- Просмотреть записанный бой по событиям с точными HP и энергией на каждом шаге.
- В консоли получить сводную статистику выбранных матчей командой `s`.
...
**Технические показатели:** 6584 содержательных строк собственного Python-кода,
341 успешно пройденных тестов, 100% исполняемых строк и ветвей пакета `arena`.
В подсчёт строк входят приложение, тесты и инструменты; пустые строки,
комментарии, docstring, данные и зависимости исключены.
Подробная [матрица критериев](docs/ACCEPTANCE.md) отделяет выполненные
технические пункты от согласований с преподавателем.
