...
def main(argv: list[str] | None = None) -> int:
    ...
    mode.add_argument(
        "--cli", action="store_true", help="Консольный интерфейс"
    )
    mode.add_argument("--test", action="store_true", help="Запустить pytest")
    parser.add_argument(
        "--data-dir",
        type=Path,
        default=Path.home() / ".battle-arena",
    ...
    if args.test:
        try:
            ...
            parser.exit(
                2, 'Установите тестовые зависимости: pip install -e ".[dev]"\n'
            )
        return int(
            pytest.main(
    ...
    try:
        ...
        if isinstance(error, tkinter.TclError):
            parser.exit(
                2, f"Не удалось открыть окно: {error}. Используйте --cli.\n"
            )
        raise
