...
from arena.domain.model import ActionOption, ActionRequest, ActionResult, BattleSnapshot, Phase
...
class GameController:
    ...
    def new_match(self, setup: MatchSetup) -> BattleSnapshot:
        first = Character("p1", setup.first.name, setup.first.archetype,
                          setup.first.equipment)
        second = Character("p2", setup.second.name, setup.second.archetype,
    ...
    def is_bot_turn(self) -> bool:
        ...
        return bool(snapshot and snapshot.mode == "bot" and
                    snapshot.active_id == snapshot.fighters[1].id and
    ...
    def submit(self, request: ActionRequest, *, from_bot: bool = False) -> ActionResult:
        if self._engine is None:
        ...
        if from_bot != self.is_bot_turn():
            return ActionResult(False, "wrong_controller", "Сейчас ход другого участника")
    ...
    def bot_step(self, match_id: str, expected_turn: int) -> ActionResult:
        snapshot = self.snapshot()
        if (not snapshot or snapshot.match_id != match_id or
                snapshot.turn != expected_turn or not self.is_bot_turn()):
            return ActionResult(False, "stale_callback", "Ответ бота устарел")
        bot = BOTS[snapshot.strategy]()
        return self.submit(bot.choose_action(snapshot, self.options()), from_bot=True)

    def save(self, slot: int) -> None:
        snapshot = self.snapshot()
        if snapshot is None:
            raise StorageError("Нет матча для сохранения")
    ...
    def load(self, slot: int) -> BattleSnapshot:
        ...
        self._engine = engine
        self.last_setup = MatchSetup(
            FighterSetup(snapshot.fighters[0].name, snapshot.fighters[0].archetype,
                         snapshot.fighters[0].equipment),
            FighterSetup(snapshot.fighters[1].name, snapshot.fighters[1].archetype,
                         snapshot.fighters[1].equipment),
            snapshot.mode, snapshot.strategy,
        )
        self._paused = False
        self.warning = ""
        self._record_result()
