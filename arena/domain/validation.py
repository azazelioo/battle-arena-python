...
def validate_snapshot(snapshot: BattleSnapshot) -> None:
    require(bool(snapshot.match_id) and len(snapshot.match_id) <= 100,
    ...
    require(snapshot.phase in (Phase.WAITING_ACTION, Phase.FINISHED),
    ...
    for fighter in snapshot.fighters:
        require(0 < len(fighter.name) <= 24 and fighter.name == fighter.name.strip(),
                "Некорректное имя")
        require(fighter.stats == final_stats(fighter.archetype, fighter.equipment),
        ...
        require(len(fighter.inventory) == len(ITEMS) and
                set(dict(fighter.inventory)) == set(ITEMS), "Неверный инвентарь")
        for _, amount in fighter.inventory:
            integer(amount, 0, 3)
        require(fighter.last_action_id in (
            "", "attack", "defend", "heal", "recover", "use_item",
            CHARACTERS[fighter.archetype].special,
        ), "Неизвестное последнее действие")
        effect_ids: set[str] = set()
        for effect in fighter.effects:
            require(effect.effect_id in ("poison", "guard"), "Неизвестный эффект")
            ...
            if effect.effect_id == "guard":
                ...
                require(not (snapshot.phase == Phase.WAITING_ACTION and
                             fighter.id == snapshot.active_id),
                        "Защита должна истечь в начале хода")
            else:
                integer(effect.remaining, 1, 2)
                require(effect.source_id != fighter.id, "Самоотравление")
        for value in (fighter.totals.direct_damage, fighter.totals.poison_damage,
                      fighter.totals.healing, fighter.totals.items_used):
            integer(value, 0, 100000)
    integer(snapshot.next_event, 1, 2000)
    require(snapshot.next_event == len(snapshot.events) + 1,
            "Нарушен номер следующего события")
    known_kinds = {
        "action", "damage", "healing", "energy", "guard_applied", "guard_used",
        "guard_expired", "poison", "poison_applied", "poison_removed",
        "item_used", "turn", "finished",
    }
    action_count = 0
    last_turn = 1
    for index, event in enumerate(snapshot.events, 1):
        ...
        require(event.actor_id in ids or
                (event.kind == "finished" and event.actor_id == ""),
                "Неизвестный автор события")
        require(event.target_id in ids, "Неизвестная цель события")
        require(event.action_id in actions or event.action_id in
                ("", "defeat", "poison", "limit", "surrender"),
    ...
    require(action_count == snapshot.successful_actions, "Счётчик действий не совпадает")
    require(bool(snapshot.events) and snapshot.events[0].kind == "turn",
    ...
    require(snapshot.active_id == expected_active, "Нарушено чередование ходов")
    dead = [f.id for f in snapshot.fighters if f.hp == 0]
    if snapshot.phase == Phase.WAITING_ACTION:
        require(not dead and snapshot.successful_actions < ACTION_LIMIT,
                "Невозможный продолжающийся бой")
        require(snapshot.turn == snapshot.successful_actions + 1,
                "Неверный номер хода")
        require(snapshot.winner_id is None and not snapshot.finish_reason,
                "Итог у незаконченного боя")
        require(not any(e.kind == "finished" for e in snapshot.events),
                "Событие завершения в продолжающемся бое")
        return
    require(snapshot.events[-1].kind == "finished", "Отсутствует событие итога")
    require(sum(e.kind == "finished" for e in snapshot.events) == 1,
            "Повторное завершение")
    require(snapshot.events[-1].actor_id == (snapshot.winner_id or "") and
            snapshot.events[-1].action_id == snapshot.finish_reason,
            "Итог не совпадает с журналом")
    reason = snapshot.finish_reason
    if reason == "limit":
        require(not dead and snapshot.winner_id is None and
                snapshot.successful_actions == ACTION_LIMIT and
                snapshot.turn == ACTION_LIMIT, "Неверная ничья")
    elif reason in ("defeat", "poison"):
        require(len(dead) == 1 and snapshot.winner_id in ids - set(dead),
        ...
        if reason == "poison":
            require(snapshot.successful_actions < ACTION_LIMIT,
                    "После лимита следующий тик не начинается")
            require(snapshot.active_id in dead, "Яд должен убить активного бойца")
        else:
            require(snapshot.active_id == snapshot.winner_id,
                    "Прямой удар должен завершиться победой атакующего")
    elif reason == "surrender":
        require(snapshot.successful_actions < ACTION_LIMIT and not dead and
                snapshot.winner_id in ids - {snapshot.active_id} and
