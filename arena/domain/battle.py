...
class BattleEngine:
    def __init__(
        ...
        if mode not in ("pvp", "bot") or strategy not in (
            "aggressive",
            "cautious",
        ):
    ...
    def _context(
        self, action: Action, item_id: str | None = None
    ) -> ActionContext:
        target = self._actor if action.self_target else self._opponent
        return ActionContext(
            self._actor, target, self._turn, action.id, self._record, item_id
        )
    ...
    def _allowed(self) -> tuple[str, ...]:
        return (
            ...
            "heal",
            "recover",
            CHARACTERS[self._actor.archetype].special,
            "use_item",
        )
    ...
    def submit(self, request: ActionRequest) -> ActionResult:
        checks = (
            (
                self._phase != Phase.WAITING_ACTION,
                "finished",
                "Бой не ожидает действия",
            ),
            (
                request.match_id != self._match_id,
                "wrong_match",
                "Команда другого матча",
            ),
            (
                type(request.expected_turn) is not int
                or request.expected_turn != self._turn,
                "stale_turn",
                "Команда устарела",
            ),
            (
        ...
        if request.action_id == "surrender":
            if (
                request.target_id != self._actor.id
                or request.item_id is not None
            ):
    ...
    def _begin_turn(self) -> None:
        context = ActionContext(
            self._actor, self._opponent, self._turn, "", self._record
        )
    ...
    def from_snapshot(cls, snapshot: BattleSnapshot) -> BattleEngine:
        # Skip __init__: WAITING_ACTION already includes start ticks.
        from .validation import validate_snapshot

        validate_snapshot(snapshot)
        engine = cls.__new__(cls)
        engine._fighters = (
            Character.from_snapshot(snapshot.fighters[0]),
        ...
        engine._active = (
            0 if snapshot.active_id == engine._fighters[0].id else 1
        )
