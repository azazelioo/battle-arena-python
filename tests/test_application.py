...
from arena.presentation.formatting import (
    RULES,
    event_text,
    fighter_text,
    result_text,
...
def test_bot_matches_always_terminate(first, second, bot_type):
    engine = BattleEngine(
        Character("p1", "A", first), Character("p2", "B", second)
    )
    bot = bot_type()
...
def test_aggressive_uses_max_damage_and_free_fallback():
    ...
    assert (
        AggressiveBot()
        .choose_action(s, engine.action_options(s.active_id))
        .action_id
        == "fireball"
    )
...
def test_cautious_antidote_before_heal_when_next_tick_lethal():
    ...
    request = CautiousBot().choose_action(
        s, engine.action_options(s.active_id)
    )
...
def test_cautious_healing_tie_prefers_spell():
    ...
    s = engine.snapshot()
    options = tuple(
        replace(o, healing=36) if o.item_id == "health_potion" else o
...
def test_controller_load_restores_setup_and_resumes(tmp_path):
    controller = GameController(tmp_path)
    setup = MatchSetup(
        FighterSetup("A", "ranger", "blade"),
        FighterSetup("B", "warrior", "shield"),
        "pvp",
        "aggressive",
    )
...
def test_controller_without_match(tmp_path):
    ...
    assert not controller.is_bot_turn()
    assert (
        controller.submit(ActionRequest("x", 1, "p1", "attack", "p2")).code
        == "no_match"
    )
...
def test_settings_failed_write_does_not_apply(tmp_path):
    ...
    with patch(
        "arena.application.controller.save_settings",
        side_effect=StorageError("disk"),
    ):
        with pytest.raises(StorageError):
...
@pytest.mark.parametrize(
    "setup",
    [
        lambda: FighterSetup(archetype="alien"),
        lambda: FighterSetup(equipment="alien"),
        lambda: MatchSetup(mode="online"),
...
def test_console_invalid_input_and_eof(tmp_path):
    ...
    output = []
    assert (
        Console(controller, lambda _: next(inputs), output.append).run() == 0
    )
...
def test_formatters_cover_battle_and_result(battle):
    ...
    assert "100" in RULES
    assert all(
        event_text(e, snapshot).startswith(f"{e.number:03d}")
